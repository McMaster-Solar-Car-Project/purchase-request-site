...
def create_user_programmatically(
    name: str, email: str, personal_email: str, address: str, team: str, password: str
):
...
def list_all_users():
    ...
    try:
        ...
        for user in users:
            ...
            print(
                f"   • {user.name} ({user.email}) - {user.team} - Signature: {sig_size} bytes"
            )
    finally:
        db.close()
