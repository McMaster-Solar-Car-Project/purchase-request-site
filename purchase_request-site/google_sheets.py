...
SHEET_TAB_NAME = os.getenv("GOOGLE_SHEET_TAB_NAME")  # Using existing tab from your sheet
...
class GoogleSheetsClient:
    ...
    def _authenticate(self):
        ...
        try:
            ...
            logger.info("Successfully authenticated with Google Sheets API using environment variables")
            return True
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if e.resp.status == 403:
                logger.error("Permission denied. Make sure you shared the sheet with: " + 
                           os.getenv("GOOGLE_SETTINGS__CLIENT_EMAIL", "service account email"))
            return False
        except Exception as e:
    ...
    def write_test_data(self) -> bool:
        ...
        try:
            ...
            test_data = [
                [current_time, "Test User", "test@mcmaster.ca", "123 Test St", "test.etransfer@email.com", "Test Team"],
                [current_time, "Another Test", "another@mcmaster.ca", "456 Demo Ave", "demo.etransfer@email.com", "Demo Team"]
            ]
            
            # Write to the sheet
            range_name = f"{SHEET_TAB_NAME}!A:F"  # 6 columns to match session data format
            body = {
                'values': test_data
            }
            ...
            logger.info(f"Test data written successfully. Updated {result.get('updates', {}).get('updatedRows', 0)} rows")
            return True
    ...
    def log_purchase_request(self, user_info: Dict[str, Any], forms: List[Dict[str, Any]], session_folder: str) -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Session data logged to Google Sheets. Updated {updated_rows} row(s)")
            return True
...
def log_purchase_request_to_sheets(user_info: Dict[str, Any], forms: List[Dict[str, Any]], session_folder: str):
    """
